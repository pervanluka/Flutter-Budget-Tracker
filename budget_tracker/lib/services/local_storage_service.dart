...
class LocalStorageService {
  ...
  Future<void> initializeHive() async {
    ...
    // Hive.deleteBoxFromDisk(budgetBoxKey);
    // Hive.deleteBoxFromDisk(transactionsBoxKey);
    // Hive.deleteBoxFromDisk(balanceBoxKey); 
  ...
  void deleteTransactionItem(TransactionItem transaction) {
    ...
    // Update the balance
    saveBalanceAfterDelete(transaction);
  }

  Future<void> saveBalanceAfterDelete(TransactionItem item) async {
    final balanceBox = Hive.box<double>(balanceBoxKey);
    final currentBalance = balanceBox.get("balance") ?? 0.0;
    balanceBox.put("balance", currentBalance + item.amount);
  }
